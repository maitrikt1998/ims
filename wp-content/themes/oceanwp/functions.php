...
final class OCEANWP_Theme_Class {
	...
	public function __construct() {
		...
		// Redirect to custom login page upon logout
		...
		function custom_authenticate_user($user, $username, $password) {
			...
			if ($is_verified != 1) {
				return new WP_Error('not_verified', 'Your email is not verified. Please check your email to verify your account.');
			}
		...
		function handle_custom_ipn_processed($ipn_data) {
				global $wpdb;
				$table_name = $wpdb->prefix . 'payment_details';

				// Add logging to see the incoming IPN data
				error_log(print_r($ipn_data, true)); // Logs to wp-content/debug.log if WP_DEBUG_LOG is enabled

				// Process and insert payment details into the database
				$user_id = isset($ipn_data['custom']) ? sanitize_text_field($ipn_data['custom']) : 0;
				$transaction_id = sanitize_text_field($ipn_data['txn_id']);
				$amount = floatval($ipn_data['mc_gross']);
				$currency = sanitize_text_field($ipn_data['mc_currency']);
				$payment_status = sanitize_text_field($ipn_data['payment_status']);
				$payment_date = isset($ipn_data['payment_date']) ? date('Y-m-d H:i:s', strtotime($ipn_data['payment_date'])) : current_time('mysql');
		...
		// add_action('after_setup_theme', 'disable_admin_bar_for_custom_dashboard');
		...
		if ( is_admin() ) {
			...
			// echo 'First Name: ' . $current_user->first_name . '<br>';
			// echo 'Last Name: ' . $current_user->last_name . '<br>';
			// echo 'Nickname: ' . $current_user->nickname . '<br>';
			// echo 'User Roles: ' . implode(', ', $current_user->roles) . '<br>';
			// echo 'User Registered Date: ' . $current_user->user_registered . '<br>';
			// echo 'User URL: ' . $current_user->user_url . '<br>';
			// echo 'User Description: ' . $current_user->description . '<br>';
			// $urole=implode(', ', $current_user->roles);
			// if($urole == "therapist"){
			// 	echo "I am a therapist";
			// }
